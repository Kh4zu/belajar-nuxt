  ...
  <div class="min-h-screen bg-[#F9FFF9] pt-28 px-6 text-gray-900 flex flex-col items-center">
    ...
    <div v-if="pembayaran" class="bg-white/90 shadow-lg rounded-3xl p-8 w-full max-w-2xl">
      ...
      <div class="space-y-4">
        <label class="flex items-center gap-3 cursor-pointer">
          <input type="radio" v-model="metode" value="Transfer Bank" />
          <span>Transfer Bank (BCA, Mandiri, BRI)</span>
        </label>

        <label class="flex items-center gap-3 cursor-pointer">
          <input type="radio" v-model="metode" value="E-Wallet" />
          <span>E-Wallet (Dana, OVO, Gopay)</span>
        </label>

        <label class="flex items-center gap-3 cursor-pointer">
          <input type="radio" v-model="metode" value="COD" />
          <span>Bayar di Tempat (COD)</span>
        </label>
      </div>

      <div class="mt-8 text-center">
        <button @click="prosesPembayaran" :disabled="!metode"
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="bg-white rounded-3xl shadow-xl p-8 w-full max-w-md text-center">
        ...
        <button @click="selesai" class="bg-[#009879] text-white font-semibold px-6 py-2 rounded-full hover:bg-[#00BFA6] transition-all">
          Kembali ke Beranda
        </button>
      </div>
    </div>
